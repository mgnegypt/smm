...
if( !route(1) && $_POST ):
  ...
  $type = htmlentities($_POST["type"]);
    ...
    $token .=  substr(md5(microtime()),rand(0,26),5);
      ...
      if( $type == "sms" ):
        $send = SMSUser($row["telephone"],"Şifrenizi değiştirmek için; ".site_url("resetpassword/$token"));
      endif;
      if( $type == "email" ):
        $send = sendMail(["subject"=>"Şifremi Unuttum.","body"=>"Şifrenizi değiştirmek için doğrulama kodunuz : ".site_url("resetpassword/$token"),"mail"=>$row["email"]]);
      endif;
